...
SWEEP_MODELS: tuple[SweepModel, ...] = (
    SweepModel(
        ...
        batch_size=12,
        notes="Cheap baseline for smoke tests and early comparison.",
    ),
    SweepModel(
        alias="qwen35-2b",
    ...
    SweepModel(
        ...
        notes="Memory outlier; use A100 for stable full runs.",
    ),
    SweepModel(
        alias="smollm2-1p7b",
...
def _command_for(model: SweepModel, *, smoke: bool, preset: str) -> list[str]:
    cmd = [
        sys.executable,
        "tuner.py",
        "cloud-pipeline",
...
def _print_commands(models: list[SweepModel], *, smoke: bool, preset: str) -> int:
    if not models:
        print("No matching sweep models found.")
        return 1

    for model in models:
        cmd = " ".join(_command_for(model, smoke=smoke, preset=preset))
        print(f"# {model.alias} -> {model.model_id}")
        print(cmd)
    return 0


def _launch(models: list[SweepModel], *, smoke: bool, preset: str) -> int:
    if not models:
        print("No matching sweep models found.")
        return 1

    for model in models:
        cmd = _command_for(model, smoke=smoke, preset=preset)
...
def main() -> int:
    ...
    if args.command == "commands":
        return _print_commands(models, smoke=args.smoke, preset=args.preset)
    if args.command == "launch":
        return _launch(models, smoke=args.smoke, preset=args.preset)
