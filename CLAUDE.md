...
### [2026-01-16] Claude Code Agent Registration - YAML Frontmatter Fix
**Problem**: Custom PACT agents in `.claude/agents/` weren't loading as Task tool subagent types. Only `pact-memory-agent` worked.
**Root Cause**: Long single-line descriptions with embedded `<example>` tags broke YAML parsing.
**Solution**: Use multiline `|` format with concise 2-line descriptions. No embedded XML.
**Correct format**:
```yaml
description: |
...
```
**Files fixed**: All 7 PACT agents in `.claude/agents/`

---

## Codebase Reference

For repository-specific commands, project structure, decision trees, diagnostics, and troubleshooting, see:
