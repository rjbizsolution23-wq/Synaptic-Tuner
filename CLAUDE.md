...
### [2026-03-14] HF Jobs Cloud Evaluation via vLLM
**Context**: After cloud training completed successfully, the next gap was evaluation. Local evaluator flows already supported Unsloth, llama.cpp, Ollama, LM Studio, and internal vLLM clients, but there was no remote HF Jobs workflow to evaluate a bucketed cloud run without first downloading it locally or converting to GGUF.
**Goal**: Make `python tuner.py cloud-eval --run latest --preset full` launch a second HF Job that evaluates the latest bucketed training run directly on remote GPU using vLLM.
**Decisions**: Added a top-level `cloud-eval` handler and main-menu entry, enabled `vllm` in `Evaluator.cli`, created `Evaluator/cloud_hf_job.py` to download the run's `final_model/` from the HF bucket, read `adapter_config.json` for the base model, start vLLM with the LoRA adapter, run `Evaluator.cli --backend vllm`, and sync results back into the source run under `evaluations/vllm/{timestamp}/`.
**Lessons**: Cloud evaluation should reuse the exact-commit launch rules from cloud training because HF Jobs still clones and runs the submitted SHA. For bucketed LoRA evaluation, the cloud job only needs `final_model/`; downloading the full training run is unnecessary. `Evaluator.cli` already had working internal vLLM support via the client factory, but its public parser was missing the `vllm` backend choice, so the remote job needed that fixed before orchestration could work. This path assumes `base_model_name_or_path` in `adapter_config.json` points to a hub-accessible model; local filesystem base-model paths are not usable inside remote HF evaluation jobs.
