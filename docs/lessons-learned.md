...
- The cloud-eval overlay must not become a second ML runtime. Installing unconstrained `peft` or transitive dependencies into `/tmp/hf-eval-site` on a stable Unsloth image can pull incompatible `torch`, `transformers`, and `numpy` versions, shadow the image stack, and fail imports such as `TrainerCallback`
