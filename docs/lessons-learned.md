...
- The cloud-eval runtime overlay must not become a second ML runtime. Installing unconstrained `peft` or transitive dependencies into the evaluator `PYTHONPATH` on a stable Unsloth image can pull incompatible `torch`, `transformers`, and `numpy` versions, shadow the image stack, and fail imports such as `TrainerCallback`
- Buckets-only Hub packages must also stay off the evaluator `PYTHONPATH`: `huggingface_hub>=1.5.0` is needed for Buckets, but stable Unsloth's `transformers 4.57.1` requires `huggingface-hub<1.0`
...
**Decisions**: Launch the eval helper with `python -m Evaluator.cloud_hf_job`, add a local cloud-eval dashboard adapter/replayer using structured JSONL progress events, add `cloud-pipeline` to train on HF Jobs and then evaluate the exact resulting run automatically, switch the HF Jobs eval runtime from vLLM to direct Unsloth inference, keep bucket sync on the isolated helper subprocess path, split evaluator runtime deps from bucket-sync deps, expose Buckets-only `huggingface_hub>=1.5.0` only through `HF_BUCKET_SYNC_PYTHONPATH`, install eval overlay helper packages with `--no-deps` and leave heavy ML packages to the selected eval image unless explicitly pinned, fix `eval_run.yaml` presets to point at `tool_prompts.yaml` and `behavior_prompts.yaml`, and standardize the saved eval artifact set as `evaluation_results.json`, `evaluation_results.md`, `evaluation_lineage.json`, and `logs/eval_progress.jsonl`.
