"""HF Jobs per-example loss stage runner for experiment orchestration.

Extracted from experiment_handler.py to keep each stage runner in its own
module.  Used by ExperimentHandler via ExperimentOrchestrator to run remote
per-example loss computation for a bucketed training run.
"""
